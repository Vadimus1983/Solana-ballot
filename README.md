...
│   │       │   ├── root_history.rs     # RootHistoryAccount (zero-copy ring buffer)
...
└── Proposal   [1 per election]
    ...
    ├── RootHistoryAccount         [1 per proposal — zero-copy ring buffer of recent Merkle roots]
...
| 3 | `store_vk` | Authority | Setup | Writes Groth16 VK to `VerificationKeyAccount`; first write only |
| 4 | `replace_vk` | Authority | Registration | Overwrites a previously stored VK; only callable while still in `Registration` phase |
| 5 | `register_commitment` | Voter | Registration | Voter deposits `PendingCommitmentRecord` with their ZK commitment |
| 6 | `register_voter` | Admin | Registration | Admin reads pending commitment, inserts leaf into Merkle tree, closes `PendingCommitmentRecord` |
| 7 | `open_voting` | Admin | Voting | Transitions proposal to `Voting`; requires VK set and `voter_count > 0`; freezes VK |
| 8 | `cast_vote` | Voter | Voting | Verifies voter-supplied `merkle_root` against `RootHistoryAccount`; Groth16 proof verified on-chain; mints `NullifierRecord` + `VoteRecord` |
| 9 | `close_voting` | Anyone | Reveal | Permissionless after `voting_end` timestamp |
| 10 | `reveal_vote` | Anyone | Reveal | Opens commitment: verifies `Poseidon(vote, randomness) == stored_commitment` |
| 11 | `finalize_tally` | Anyone | Finalize | Locks `yes_count`/`no_count`; callable when all revealed or 24 h grace expired |
| 12 | `close_vote_accounts` | Anyone | Cleanup | Closes `NullifierRecord` + `VoteRecord`; rent to `refund_to` address |
| 13 | `close_commitment_record` | Anyone | Cleanup | Closes `CommitmentRecord` + `VoterRecord`; terminal status required |
| 14 | `close_proposal` | Admin | Cleanup | Closes `Proposal`; requires all vote/commitment accounts closed first |
| 15 | `close_vk` | Admin | Cleanup | Closes `VerificationKeyAccount`; returns ~0.00631 SOL rent to admin; terminal status required |
| 16 | `expire_proposal` | Anyone | Cleanup | Advances stuck `Registration` proposals to `Expired` after `voting_end` |
...
| `ROOT_HISTORY_SIZE` | 32 | Roots retained in `RootHistoryAccount`; proof valid across last 32 registrations |
...
4. Upload the VK on-chain per proposal via `store_vk` (~0.00631 SOL deposit). If the wrong key is uploaded, it can be overwritten with `replace_vk` while the proposal is still in `Registration` phase. Once `open_voting` is called the key is permanently frozen.
...
```
Admin      →  open_voting()
Voter(i)   →  cast_vote(proof, nullifier, vote_commitment, merkle_root, refund_to)
...
4. Fetch `merkle_root` from the on-chain `Proposal` account at any point during or after registration — proofs generated against any of the last `ROOT_HISTORY_SIZE` (32) roots are accepted on-chain.
5. Build a 20-level Merkle inclusion proof for your commitment against that root.
6. Run the Groth16 prover → 256-byte proof.
7. Submit `cast_vote` (requires `SetComputeUnitLimit ≥ 1 400 000 CU`).

On-chain, the program:
- Checks the voter-supplied `merkle_root` is present in the `RootHistoryAccount` ring buffer (or equals the current frozen root).
- Validates the 256-byte proof against the stored VK and the supplied Merkle root.
...
| Role | Who | Can Call |
|---|---|---|
| **Program Authority** | Deployer wallet (`PROGRAM_AUTHORITY`) | `initialize`, `create_proposal`, `store_vk`, `replace_vk` |
...
| `RootHistoryAccount` | 1 033 B | **~0.00808 SOL** | Admin | ✅ Yes | `close_proposal` → admin |
...
- `cast_vote` requires `SetComputeUnitLimit ≥ 1 400 000 CU`; the Groth16 alt_bn128 pairing syscall alone consumes ~1 M CU. The React client automatically prepends `ComputeBudgetProgram.setComputeUnitLimit({ units: 1_400_000 })` via `.preInstructions([...])`. CLI or custom callers must include this instruction explicitly.
...
### 3. `register_voter` Is Not Batched
...
### 5. ~~Merkle Root Race Condition~~ *(Fixed)*

`RootHistoryAccount` stores a zero-copy ring buffer of the last `ROOT_HISTORY_SIZE` (32) Merkle roots produced by `register_voter`. `cast_vote` accepts a proof generated against any root in this buffer, so voters no longer need to time their proof generation to the final registration. A proof stays valid across up to 32 subsequent registrations after the voter fetched the root.
